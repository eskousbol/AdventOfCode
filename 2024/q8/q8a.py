  ...
  for frequency in antennae.keys():
    ...
    for i in range(len(positions) - 1):
      ...
      for j in range(i + 1, len(positions)):
        ...
        antinode_1 = update_position(antenna_1, antenna_directions)
        antinode_2 = update_position(antenna_2, [-antenna_directions[0], -antenna_directions[1]])

        if on_grid(antinode_1, grid_width, grid_height) and is_valid_antinode(antenna_1, antenna_2, antinode_1):
          antinodes.add(antinode_1)
  
        if on_grid(antinode_2, grid_width, grid_height) and is_valid_antinode(antenna_1, antenna_2, antinode_2):
